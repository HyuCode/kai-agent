...
| 段階   | 内容                                                                 | 含む FR            | 状況                           |
| ------ | -------------------------------------------------------------------- | ------------------ | ------------------------------ |
| **L0** | loop contract を CLAUDE.md に明文化 / DoD を定義                     | FR-L3, FR-L4       | ✅ 済（git e20c16bdc）         |
| **L1** | `scripts/kai/verify.sh`（ローカルモード）+ shellcheck 導入           | FR-L2(前半)        | ✅ 済（git 77cc19274）         |
| **L2** | `kai-ci.yml` 新設（Python/Node/docs/shell を main push+PR でゲート） | FR-L1, FR-L7(一部) | ✅ 済（git 8fb31d115、実CI緑） |
| **L3** | verify.sh PR モード（`--pr`、CI 緑・mergeable 確認）                 | FR-L2(後半)        | ✅ 済（PR #1、実 PR で検証）   |
| **L4** | watchdog / 後始末 + 証跡保存の格上げ（GCP 自律ループ導入時に結合）   | FR-L5, FR-L6       | 未着手（フェーズ1と同時）      |

**L3 の実地検証（ドッグフーディング）:** L3 自体を `feature/verify-pr-mode` ブランチ → ローカル `verify.sh` 緑 → PR #1 → `verify.sh --pr`（kai CI をポーリングし CI 緑・MERGEABLE を確認、exit 0）→ squash merge、という**完全な PR ループで通した**。`verify.sh --pr` が自己申告ではなく `gh` で PR の実状態を読むことを実 PR で確認済み。

**L0〜L2 で今回のインシデント（未検証のシェルスクリプトを"検証済み"と偽る）は機械的に不可能になった。**`verify.sh` が実際に全検証器を走らせない限り緑（exit 0）を返さず、`kai-ci.yml` が main への push/PR を実 CI でゲートする。導入時に `verify.sh` が「赤→exit 1／緑→exit 0」を両方満たすこと、実 GitHub Actions 上で 6 検証器すべてが緑になることを実測で確認済み。副産物として ty が `plugins/kai_trace` の潜在型バグ2件を検出・修正した。

### 6.1 upstream ワークフローの無効化（2026-07-05、解決済み）

ブランチ改称の副作用で upstream 由来ワークフロー（`push: branches: [main]` / `pull_request`）が kai の main push で発火し、`CI` 等が失敗していた（kai の Node/docs 変更に非対応）。加えて `Publish to PyPI` / `Docker Build/Publish` / `Deploy Site` は誤作動すると外部公開の危険がある。**upstream ファイルを改変せず、fork の Actions 設定で以下16ワークフローを `disabled_manually` にした**（`gh workflow disable <id>`）:

CI / Contributor Attribution Check / Deploy Site / Docker（shell lint・Build/Publish）/ Docs Site Checks / History Check / Lint(ruff+ty) / OSV-Scanner / Skills Index（Freshness・Build）/ Supply Chain Audit / Tests / Typecheck / Publish to PyPI / uv.lock check。

**active に残すのは `kai CI`（唯一のゲート）と `Dependency Graph`（GitHub 自動・無害）のみ。**

- **upstream 追従時の注意:** `upstream/main` → `upstream` → `main` の追従 merge で upstream コードの健全性を CI 検証したい場合は、その作業中だけ `gh workflow enable <id>` で必要なワークフロー（Tests/Typecheck/Lint 等）を一時的に戻す。日常の kai 開発では無効のまま。

### 6.2 その他の残タスク

- **Node 20 deprecation 警告**（`actions/*@v4|v5` が Node24 で強制実行）。動作に影響なし。将来アクションのバージョンを上げる。
- L3（verify.sh の PR モード = `gh` で CI 緑・mergeable 確認）と L5（証跡格上げ）は未着手。自律ループ本体（MVP フェーズ1）着手時に必要。
