...
- **第 2 回リハーサル記録（2026-07-05）:** 限定公開で 22 分実施（`test-live-02.md`）。
  ...
  #31（実況の質）/ #32（VSCode ステージ整備）。次回はこれらの解消後、1 時間実施
...
kai_director → kai-typewriter 拡張がタイピング風に再生し、コマンドと応答は
統合ターミナル内の tmux セッションに映る。

```bash
# 初回のみ: bash ~/kai-agent/kai-services/streaming/vm/setup-vscode.sh
tmux new-session -d -s kai-stream -x 200 -y 24 "cd ~/kai-agent && exec bash"
systemd-run --user --unit=kai-vscode --collect --setenv=DISPLAY=:0 \
  code --disable-gpu --wait ~/kai-agent
# VSCode 内で Ctrl+` → 統合ターミナルで: tmux attach -t kai-stream
# VSCode を最大化し、拡張の疎通を確認:
curl -s -X POST http://127.0.0.1:8920/edit -d '{"files": []}'   # {"queued":0}
```
