...
- **upstream `ci.yml` が kai の main push で発火して失敗する。** ブランチ改称の副作用。upstream ワークフローは kai の Node/docs 変更に対応しておらず赤くなる（ノイズ）。upstream ファイルを改変せず、fork の Actions 設定で該当ワークフローを無効化するか、`kai CI` のみを required check にする運用で切り分ける（FR-L1 の残作業。要オーナー判断）。
