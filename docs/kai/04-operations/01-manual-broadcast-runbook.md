...
- **第 2 回リハーサル記録（2026-07-05）:** 限定公開で 22 分実施。
...
- **第 3 回リハーサル記録（2026-07-06）:** 限定公開で 7 分実施。
...
- **前提:** [`../01-product/02-mvp-roadmap.md`](../01-product/02-mvp-roadmap.md) §M4。操作はすべてオーナーが手動実行（MVP）
- **場所:** 特記なければ kai-vm 上（`ssh kai@<kai-vm>` または UTM ウィンドウ内ターミナル）

## 1. ゴール（この台本で満たす DoD）

`../01-product/02-mvp-roadmap.md` §1.1 の 5 条件を 1 時間のリハーサル配信（**限定公開**)で通しで満たす:
