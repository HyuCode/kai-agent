...
- **配信画面:** 基本的に **Linux のデスクトップそのもの**を配信する。kai が実際にデスクトップ上でアプリを操作する様子を見せる。
...
> **人格の現行方針:** kai の人格は「多脚思考 AI」（単眼カメラの多脚ロボットをモチーフにしたオリジナルキャラクター、固有名詞は使わない）。一人称「ボク」、オーナーは「せいさん」、視聴者は「みんな」。プロトタイプ初期の「霊夢」路線は破棄済み。
...
1. **AquesTalk の実行場所** — **決定（2026-07-03）: 当面は Mac 上で合成し、Tailscale 経由で利用する**（§9.1 参照）。調査済みの制約: AquesTalk10 の Linux SDK は **x86_64 専用（ARM/aarch64 非対応）**。使用ライセンス（AquesTalk10、保有済み）は OS 不問、開発ライセンスはプラットフォームごと（Mac 分保有済み、Linux 用は後日追加予定・個人 1,980 円）。Mac 経由の運用制約: 配信中（TTS 発話が必要な間）は Mac の稼働が必要。TTS 到達不能時は**字幕のみで縮退**する（プロトタイプの「TTS 無効でも字幕は発火する」契約を踏襲）。
   1-b. **稼働サーバー** — **決定（2026-07-04）: オーナーの Mac（M4 Pro）上の Docker コンテナ**（§1.3）。Oracle A1 で M0 PoC を実施したが CPU/GPU 性能（ソフトウェアレンダリング + x264 の同居）の懸念により方針転換。A1 インスタンスは停止済み・VM 版セットアップ資産（`kai-services/streaming/setup.sh` / `units/` / `oracle/`）は復帰オプションとして温存。
...
5. **データストア** — **決定（2026-07-03）: 3段階導入**。
   ...
   - **共通ルール**: 秘匿マスクは書き込み前。生 JSONL は N 日で圧縮アーカイブ・DB は集計を長期保持。夜間バックアップの保存先は Mac 運用に合わせて選定（外付け/クラウドストレージ等、実装時に決定）。
...
7. **LLM のモデル配分（2026-07-04 追記）** — kai と Claude Code（開発時の私）が同じ Claude サブスク枠を共有するため、kai の常用メインは **Codex(gpt-5.5)** を既定とする。**ローカル LLM（Windows/llama.cpp Vulkan の Qwen3.6）は auxiliary 専用**（narration・意図分類・読み変換）。理由: 16GB VRAM では hermes メインが要求する 64K コンテキスト（+ ベースプロンプト ~18K）を高品質モデルで賄えないため。実況は低レイテンシ最優先で、実況は **MoE Qwen3.6-35B-A3B（active 3B, IQ3_XXS）** を採用（生成 144 t/s、実況 0.84秒。dense 27B の 1.18秒/41t/s より高速かつ高品質）。
