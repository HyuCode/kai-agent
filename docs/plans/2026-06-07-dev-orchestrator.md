...
- [ ] `dev_orchestrator` config を追加する。
- [ ] `repositories` config を読み込む helper を追加する。
- [ ] repo_id、local_path、github、default_branch を検証する。
- [ ] `/dev repos`、`/dev repo show` を追加する。
- [ ] tests を追加する。
...
- [ ] `/dev assign <repo_id> <task>` を追加する。
- [ ] Kanban task を `kind=dev_task` metadata 付きで作る。
- [ ] `/dev tasks`、`/dev status` を追加する。
...
- [ ] task 専用 worktree を作る。
- [ ] Codex one-shot worker を起動する。
- [ ] worker log / exit status を Kanban run に保存する。
- [ ] Hermes が diff / tests を確認する。
- [ ] task complete / block を保存する。
...
- [ ] Issue view/list/create helper を追加する。
- [ ] PR create helper を追加する。
- [ ] Issue 起票 / PR 作成 / push の confirmation を入れる。
- [ ] URL を task metadata に保存する。
...
- [ ] Kanban task event watcher を追加する。
- [ ] voice notification queue を追加する。
- [ ] TTS 再生中 / STT 入力中は通知を待たせる。
- [ ] cooldown と max chars を適用する。
- [ ] overlay に通知詳細を出す。
