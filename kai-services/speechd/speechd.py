...
設計の正典: docs/kai/02-architecture/01-system.md
...
CONNECT_TIMEOUT = 3.0
TOTAL_TIMEOUT = 30.0
# 文間の無音（ms）。一気読み感を消す「息継ぎ」（docs/kai/02-architecture/04-tts-reading-rules.md §5.3）
...
# 注意: この 3 実装は plugin/サービス単体完結の原則でコピーになっている。
# パターンや収集ロジックを変えるときは 3 箇所（kai_narrator / kai_trace /
# speechd）を同時に更新する（Issue #77 H-b）。
...
# 秘密の正典 ~/.hermes/.env（hermes は資格情報を環境変数に載せず .env 直読みで
# 解決するため、os.environ だけでは env 秘密層が実行時に空になる。Issue #77 H-b）
DOTENV_FILE = Path(_env("SPEECHD_DOTENV", str(Path.home() / ".hermes/.env")))


def _iter_dotenv_items():
    """~/.hermes/.env の KEY=VALUE を直接読む（standalone・stdlib のみ）。"""
    try:
