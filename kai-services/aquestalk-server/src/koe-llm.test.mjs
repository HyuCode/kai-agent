...
test("generateKoeLlm: apiKey 設定時は Authorization ヘッダを送る（koe-bridge 認証 #77 C1）", async () => {
  const seen = {};
  const spyFetch = async (_url, init) => {
    seen.headers = init.headers;
    return { ok: true, status: 200, json: async () => ({ choices: [{ message: { content: "てすと。" } }] }) };
  };
  await generateKoeLlm("テスト", { ...LLM_CONFIG, apiKey: "sharedtoken123", fetchImpl: spyFetch });
  assert.equal(seen.headers.Authorization, "Bearer sharedtoken123");
  // 未設定なら付けない（llama.cpp 等の認証なしバックエンドをそのまま使える）
  await generateKoeLlm("テスト", { ...LLM_CONFIG, fetchImpl: spyFetch });
