...
`docs/kai/03-features/02-narration/04-design.md`（§0 背骨・§2.2 fixture・§2.5 評価器）、
受け入れ基準は `docs/kai/03-features/02-narration/03-requirements.md`（FR1〜9）。
...
## fixture 形式（`docs/kai/03-features/02-narration/90-fixtures/*.jsonl`）
...
- fixture は実リハーサル trace（VM `~/.hermes/kai_trace/`）＋当時の Issue 本文から
  再構成（`docs/kai/03-features/02-narration/90-fixtures/` の 2 本）。増やしていく。
...
- core（`run_agent.py` 等）は不変。すべて `kai-services/narration-eval/` と
  `docs/kai/03-features/02-narration/` 配下（NFR3 Footprint Ladder）。
